...
class ChatBusiness:
    ...
    async def process_llm_task(request: ChatTriggerRequest) -> str:
        ...
        """
        logger.info(f"Processing LLM task for chat {request.chat_id} using provider: {request.provider}")
        ...
        try:
            ...
            logger.error(f"Error processing LLM task for chat {request.chat_id}: {str(e)}")
